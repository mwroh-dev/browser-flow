...
Runtime commands execute from `{{BROWSER_FLOW_CLI}}`.
...
`{{BROWSER_FLOW_AGENT_ROOT}}/{name}/AGENT.md` and is loaded as a projected view
at phase entry.
...
skill entry, load `{{BROWSER_FLOW_AGENT_ROOT}}/orchestrator/AGENT.md` and
`{{BROWSER_FLOW_AGENT_ROOT}}/orchestrator/openai.yaml` as the entry projected view.
...
2. Check `{{BROWSER_FLOW_RUNTIME_ROOT}}/knowledge/registry/workflows.json` for
...
3. Execute the **callable tool** for that phase from
   `{{BROWSER_FLOW_RUNTIME_ROOT}}/scripts/cli.mjs`.
...
Projected view:
- `{{BROWSER_FLOW_AGENT_ROOT}}/capture/AGENT.md`
- `{{BROWSER_FLOW_AGENT_ROOT}}/capture/openai.yaml`
...
Callable tool:
```
node {{BROWSER_FLOW_CLI}} prepare --run-id <id> --fixture <fixture> [--start-url <url>] [--unmasked] [--snapshot-dom]
[await_capture checkpoint — user performs the demo]
node {{BROWSER_FLOW_CLI}} done --run-id <id>
node {{BROWSER_FLOW_CLI}} done --run-id <id> --capture-screenshot final   # when the user explicitly asked for a screenshot
[capture_noise_review checkpoint — only when captureDiagnostics.status === "needs_review"]
node {{BROWSER_FLOW_CLI}} review-noise --run-id <id>
node {{BROWSER_FLOW_CLI}} review-noise --run-id <id> --apply <capture-noise-result.json>
[locator_intent_review checkpoint — only when locatorDiagnostics.status === "needs_review"]
node {{BROWSER_FLOW_CLI}} review-locator-intent --run-id <id>
node {{BROWSER_FLOW_CLI}} review-locator-intent --run-id <id> --apply <locator-intent-result.json>
[route_intent_review checkpoint — only when routeDiagnostics.status === "needs_review"]
node {{BROWSER_FLOW_CLI}} review-route-intent --run-id <id>
node {{BROWSER_FLOW_CLI}} review-route-intent --run-id <id> --apply <route-intent-result.json>
```
...
Projected view:
- `{{BROWSER_FLOW_AGENT_ROOT}}/analyzer/AGENT.md`
- `{{BROWSER_FLOW_AGENT_ROOT}}/analyzer/openai.yaml`
- `references/replay-permission-policy.md`
...
Callable tool:
```
node {{BROWSER_FLOW_CLI}} analyze --run-id <id>
```
...
Projected view:
- `{{BROWSER_FLOW_AGENT_ROOT}}/generator/AGENT.md`
- `{{BROWSER_FLOW_AGENT_ROOT}}/generator/openai.yaml`
...
Callable tool:
```
node {{BROWSER_FLOW_CLI}} generate --run-id <id>
```

### Phase 4 — Verify

Projected view:
- `{{BROWSER_FLOW_AGENT_ROOT}}/verifier/AGENT.md`
- `{{BROWSER_FLOW_AGENT_ROOT}}/verifier/openai.yaml`
...
Callable tool:
```
node {{BROWSER_FLOW_CLI}} verify --run-id <id> [--headless]
```
...
- `{{BROWSER_FLOW_SKILL_ROOT}}/scraping-agent/SKILL.md` — establishes the extractor config
- `{{BROWSER_FLOW_SKILL_ROOT}}/extract-heal-agent/SKILL.md` — repairs a drifted config
...
node {{BROWSER_FLOW_CLI}} extract --run-id <id> --step <n> --schema <targetSchema.json>
...
node {{BROWSER_FLOW_CLI}} extract --run-id <id> --apply artifacts/runs/<id>/scrape-result.json
...
node {{BROWSER_FLOW_CLI}} extract --run-id <id> --step <n> --reuse [--paged]

# 3. self-repair — when status is "drift"
#    bf extract emits extract-heal-request.json → dispatch extract-heal-agent →
node {{BROWSER_FLOW_CLI}} extract-heal --run-id <id> --apply artifacts/runs/<id>/extract-heal-result.json
...
- `{{BROWSER_FLOW_RUNTIME_ROOT}}/scripts/` commands are internal implementation — do
  not expose their structure to the user.
- `{{BROWSER_FLOW_RUNTIME_ROOT}}/knowledge/registry/workflows.json` is the catalog of
  verified workflows — read it before recommending re-capture.
- `{{BROWSER_FLOW_RUNTIME_ROOT}}/artifacts/` contains immutable run evidence — do not
  modify it.
- `{{BROWSER_FLOW_AGENT_ROOT}}/{name}/AGENT.md` files are loaded as projected views at
...
- `references/registry-contract.md` — when reading or updating
  `{{BROWSER_FLOW_RUNTIME_ROOT}}/knowledge/registry/`
