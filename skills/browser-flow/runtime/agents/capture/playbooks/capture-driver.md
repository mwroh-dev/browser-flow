...
- `Bash` for CLI invocation. The repo root is the working
  directory; the CLI entry is `node scripts/cli.mjs <command>`.
...
1. **Prepare**: run `node scripts/cli.mjs prepare --run-id <id>
...
4. **Done**: run `node scripts/cli.mjs done --run-id <id>`.
   Capture security report.
5. **Analyze**: run `node scripts/cli.mjs analyze --run-id <id>`.
   Capture workflow.json path.
6. **Generate**: run `node scripts/cli.mjs generate --run-id
...
8. **Doctor**: run `node scripts/cli.mjs doctor` (no run-id —
